...
**Новый способ доступа через InferenceClient**

Проект использует современный способ доступа к Hugging Face API через библиотеку `@huggingface/inference` и `InferenceClient`. Это обеспечивает:

- ✅ **Единый API** для работы с различными моделями
- ✅ **Автоматическую обработку** загрузки моделей
- ✅ **Поддержку множества провайдеров** (Replicate, Fal.ai, и др.)
- ✅ **Улучшенную обработку ошибок** и таймаутов
- ✅ **Совместимость с OpenAI-подобным API**
...
- ✅ Проект использует новый `InferenceClient` из `@huggingface/inference` для более надежной работы
